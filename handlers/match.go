...
func RecordBall(c *gin.Context) {
	...
	if req.DismissalType != nil && !validDismissals[*req.DismissalType] {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid dismissal_type"})
		return
	}
	if req.IsWicket && req.DismissalType == nil {
